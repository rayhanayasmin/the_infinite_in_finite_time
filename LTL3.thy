...
fun lsatisfying_AiF\<^sub>3 :: \<open>'a \<Rightarrow> 'a AiF\<^sub>3\<close> (\<open>lsat\<^sub>3\<bullet>\<close>) where
  \<open>lsat\<^sub>3\<bullet> x T = compr (\<lambda>t. t \<noteq> \<epsilon> \<and> x \<in> (thead t))\<close>
| \<open>lsat\<^sub>3\<bullet> x F = compr (\<lambda>t. t \<noteq> \<epsilon> \<and> x \<notin> (thead t))\<close>
...
\<open>triv_true x = (\<forall>s. x\<in> s)\<close>

fun triv_false :: \<open>'a \<Rightarrow> bool\<close> where
\<open>triv_false x = (\<forall>s. x\<notin> s)\<close>

fun nontrivial :: \<open>'a \<Rightarrow> bool\<close> where
\<open>nontrivial x = ((\<exists>s. x\<in> s) \<and> (\<exists>t. x\<notin> t))\<close>
...
| \<open>progress (prop\<^sub>l(a)) \<sigma> = (if a \<in> \<sigma> then true\<^sub>l else not\<^sub>l true\<^sub>l)\<close>
