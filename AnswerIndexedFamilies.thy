...
type_synonym 'a state = \<open>'a set\<close>
datatype answer = T | F
...
fun satisfying_AiF :: \<open>'a \<Rightarrow> 'a state AiF\<close> (\<open>sat\<bullet>\<close>) where
  \<open>sat\<bullet> x T = {state. x \<in> state}\<close>
| \<open>sat\<bullet> x F = {state. x \<notin> state}\<close>
...
| \<open>\<Gamma> \<Turnstile>\<^sub>p prop\<^sub>p(a) = (a \<in> \<Gamma>)\<close>
